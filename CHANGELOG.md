...
- `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_DEPLOYMENT`, `AZURE_OPENAI_API_VERSION`
  and `AZURE_TENANT_ID` from `deploy/carelockconsulting.env.example` and
  `scripts/setup.sh`.
...
- The canvas is sized by explicit `width/height: 100% !important` rather than
  ...
  inline media.
- Brightness is console-only via `window.CarelockRain.setConfig`
  (`carelock-rain-brightness` in localStorage), matching the existing
  `CarelockChaos` precedent, rather than morpheus's Settings-page control —
...
- State persists under `carelock-nfr-detail-collapsed`, alongside the filters
...
- Collapsed state persists per browser in `localStorage` under
  `carelock-nfr-filters-collapsed`, matching the pattern the global side-nav
  collapse already uses (`carelock-nav-collapsed` in
...
- **A missing typesetter returns the sources, not an error.** The app ships as
  ...
  `X-Carelock-Reason` header, and with typst 0.15.1 on `TYPST` the same request
...
- `setup.sh` and `update.sh` now install the root `*.md` files to `<bindir>/../share/carelockconsulting`, which is one of the searched locations, so no configuration is needed. `DOCS_DIR` is documented in `deploy/carelockconsulting.env.example` and `RUNTIME_ARGS.md`.
...
- `deploy/carelockconsulting.env.example` documents `REPORTING_CHROME_PATH` and `REPORTING_CHROME_NO_SANDBOX`, which the reporting module has always read but which were never listed.
...
- Cadence knobs are read from `localStorage` on every tick and exposed as `window.CarelockChaos` for console tuning (`carelock-chaos-interval`, default 60s; `carelock-chaos-density`, default 2 characters per 300 on screen). There is no settings UI — morpheus drives these from its Settings view, which this app has no equivalent of.
...
- New `internal/app/utilities.go` renders the page and wires `GET /utilities/export` (streams the snapshot as a `carelock-data-export-<timestamp>.json` download) and `POST /utilities/import` (accepts a multipart file upload, validates the snapshot version, runs the overwrite, and returns a per-table row-count report). All three routes sit behind the admin middleware (open only in `-local-mode`) because the snapshot includes password hashes and import is destructive.
...
- Added `update.sh` — redeploy script mirroring the morpheus pattern: `git pull --ff-only`, rebuild `./cmd/api` into a temp binary and `sudo install` to `/usr/local/bin/carelockconsulting`, then `sudo systemctl restart carelockconsulting`. Env overrides: `CARELOCK_ENV_FILE`, `CARELOCK_BIN_PATH`, `CARELOCK_SERVICE_NAME`. No migration step needed — schema changes apply automatically on startup.

## 2026-06-28 (PostgreSQL migration)

- Switched the Morpheus deployment from SQLite to a local PostgreSQL backend (`carelockconsulting` role/database on `localhost:5432`). Existing SQLite data synced to PostgreSQL via `scripts/db-sync.sh push` before cutover. Service env updated with `DATABASE_URL`; `SQLITE_PATH` removed.

## 2026-06-28 (nginx / service deployment)

- Added `deploy/carelockconsulting.nginx` — nginx reverse-proxy site config routing `carelockconsulting.l3d.local` → `127.0.0.1:8081`.
- Updated `deploy/carelockconsulting.env.example`: default `LISTEN_ADDR` changed to `127.0.0.1:8081` (loopback, behind nginx) and `TRUST_PROXY=true` added.
- App deployed as a systemd service (`carelockconsulting.service`) on Morpheus, co-hosted with the morpheus app; nginx routes by `server_name` (`carelockconsulting.l3d.local` → :8081, `morpheus.l3d.local` → :8080).
...
- Added a bare-metal install script `scripts/setup.sh` (modeled on the
  ...
  it to `/usr/local/bin/carelockconsulting`, creates the `carelockconsulting`
  system user and the `/var/lib/carelockconsulting` SQLite data directory,
...
- Added the systemd unit `deploy/carelockconsulting.service` (StateDirectory
  for the SQLite dir, `ProtectSystem=strict`, `CAP_NET_BIND_SERVICE` for low
  ports) and `deploy/carelockconsulting.env.example` documenting the runtime
...
- Branded every browser-tab page title with the `<Page> · CareLock Consulting`
...
- Added a branded footer to the home page (`/`, `internal/app/home.go`):
  `© <year> CareLock Consulting · Risk & Control Self-Assessment platform`,
...
- Added a top-level `README.md` branded `carelockconsulting`: quick start
...
- Renamed the Go module and output binary from `go_rcsa` to `carelockconsulting`:
  updated the `module` directive in `go.mod`, every `carelockconsulting/internal/...`
  ...
  `scripts/*` helpers, the `/carelockconsulting` entries in `.gitignore`, and the
  docs (`Agents.md`, `RUNTIME_ARGS.md`). Updated the two human-facing UI titles in
  `internal/app/home.go` to "CareLock Consulting …". `go fmt` re-grouped imports in
...
- Goreleaser now uses the real `carelockconsulting` binary name in release artifacts, and the release build script resolves Darwin cross-compilers from `.toolchains/`, `DARWIN_TOOLCHAIN_BIN_DIR`, or `PATH` instead of a machine-specific osxcross path.
